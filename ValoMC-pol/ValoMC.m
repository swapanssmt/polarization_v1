...
%          layer   layer to which that particular element belongs to.
...
%     rnseed   Random number generator seed
    ...
    if(isfield(vmcmedium,'ny') || isfield(vmcmedium,'nz'))
        vmcmedium.layer = duplicateArray(vmcmedium.layer(:), length(vmcmesh.H));
    end
    if(isfield(vmcmedium,'ny') || isfield(vmcmedium,'nz'))
    ...
    nangles = int64(1000);
    activate_pol=int64(0);
    s0=double([1;1;0;0]);
    s11=double([zeros(nangles+1,1) zeros(nangles+1,1)]);
    s12=double([zeros(nangles+1,1) zeros(nangles+1,1)]);
    s33=double([zeros(nangles+1,1) zeros(nangles+1,1)]);
    s43=double([zeros(nangles+1,1) zeros(nangles+1,1)]);
    ...
    if(exist('vmcoptions')==1)
        ...
        % start modify
        if(isfield(vmcoptions, 's11'))
            s11 = double(vmcoptions.s11);
        end
        if(isfield(vmcoptions, 's12'))
            s12 = double(vmcoptions.s12);
        end
        if(isfield(vmcoptions, 's33'))
            s33 = double(vmcoptions.s33);
        end
        if(isfield(vmcoptions, 's43'))
            s43 = double(vmcoptions.s43);
        end
        if(isfield(vmcoptions, 's0'))
            s0 = double(vmcoptions.s0);
        end
        if(isfield(vmcoptions, 'nangles'))
            nangles = int64(vmcoptions.nangles);
        end
        if(isfield(vmcoptions, 'activate_pol'))
            activate_pol = int64(vmcoptions.activate_pol);
        end
        % end modify
    ...
    if(dimensionality == 2)
        ...
        if(isfield(vmcoptions,'export_filename'))        
            ...
            fprintf(fp, '%d %d %d %d %d %d %d\n', size(s11, 1), size(s12, 1), size(s33, 1), size(s43, 1), size(s0, 1),nangles,activate_pol);
            ...
            fprintf(fp, '%18.10f\n', s0');
            % modify end
            fprintf(fp, 'mua mus g n layer\n');
            fprintf(fp, '%18.10f %18.10f %18.10f %18.10f %18.10f\n', [ mua(:) mus(:) g(:) n(:) layer(:)]');
            fprintf(fp, 'BCType\n');
            fprintf(fp, '%c\n', BCType);
            if exist('BCn')
                fprintf(fp, 'BCn\n');
